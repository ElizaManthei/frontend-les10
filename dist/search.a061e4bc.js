...
})({"1RaBv":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "c6d25140a061e4bc";
"use strict";
...
},{}],"41Y6v":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
var _axios = require("axios");
var _axiosDefault = parcelHelpers.interopDefault(_axios);
console.log("Hallo daar !");
async function aa(countryName) {
    try {
        const response = await (0, _axiosDefault.default).get(`https://restcountries.com/v2/name/${countryName}`);
        return response.data;
    } catch (e) {
        console.error("nie istnieje");
    }
}
...
//     list.innerText = emptyList.join(`<li>`);
//     item.value = '';
...
function printInfo(countryName) {
    aa(countryName).then((result)=>{
        ...
        textInfo3.innerText = ofLanguages(result[0]);
        ...
        console.log(ofLanguages(result[0]));
    }).catch((error)=>{
        console.log(error);
    });
}
...
function valuta(land) {
    ...
    if (currencies.length > 1) for(let i = 1; i < currencies.length; i++)list += ` and ${currencies[i].name}'s`;
    return list;
}
function ofLanguages(land) {
    const { languages  } = land;
    let listOfLanguages = `They speak ${languages[0].name}`;
    if (languages.length > 1) {
        for(let i = 1; i < languages.length; i++)if (i === languages.length - 1) listOfLanguages += ` and ${languages[i].name}`;
        else listOfLanguages += `, ${languages[i].name}`;
    }
 ...
 //                 const flag = document.createElement('img');
 //                 flag.src = land.flags.png;
 //                 flag.alt = 'blad';
 //                 flag.setAttribute('class', 'flag');
 //                 pr.appendChild(flag);
 //                 const landName =document.createElement('p');
 //                 landName.textContent = land.name.common;
 //                 pr.appendChild(landName);
 //                 const population = document.createElement('p');
 //                 population.textContent = `Has a population of ${land.population} people`;
 //                 pr.appendChild(population);
 //                 pr.setAttribute('class', regioColor(land.continents[0]));
 //                 id.appendChild(pr);
 //             });
 //         })
 //         .catch((error) => {
...
},{"axios":"jo6P5","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["1RaBv","41Y6v"], "41Y6v", "parcelRequire17d8")

//# sourceMappingURL=search.a061e4bc.js.map
